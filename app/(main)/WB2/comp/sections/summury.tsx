...
export default function Summary({ study }: { study: any }) {
    ...
    const applyStyle = (style: string) => {
        ...
        switch (style) {
            ...
            case 'highlight-yellow':
                span.style.backgroundColor = '#78350f';
                break;
            case 'highlight-green':
                span.style.backgroundColor = '#14532d';
                break;
            case 'highlight-red':
                span.style.backgroundColor = '#7f1d1d';
                break;
            case 'highlight-blue':
    ...
    return (
        <div className="max-w-4xl mx-auto px-6 py-12 space-y-12 text-neutral-200">
            <h1 className="text-4xl font-bold text-neutral-100">📘 Lesson Summary</h1>
            ...
            {showToolbar && (
                <div
                    className="fixed z-50 flex flex-wrap gap-2 bg-neutral-800 text-white p-3 rounded-md shadow-lg border border-neutral-600"
            ...
            {lesson.map((section: any, index: number) => (
                <div key={index} className="space-y-4">
                    <h2 className="text-2xl font-semibold text-neutral-100 border-l-4 border-blue-500 pl-5">
                        {section.title}
                    </h2>
                    <div className="space-y-4 pl-8 mt-2">
                        {section.lines.map((line: any, idx: number) => {
                            ...
                            return (
                                <div
                                    key={idx}
                                    className={`rounded-lg px-6 py-4 text-base leading-relaxed border border-neutral-700 transition-colors duration-200 ${highlighted[lineKey] ? 'bg-green-950 text-white' : 'bg-neutral-800 text-neutral-200'
                                        }`}
                                >
                                    ...
                                    {expanded[lineKey] && (
                                        <p className="mt-3 text-neutral-300 text-base border-t border-neutral-700 pt-3">
                                            {line.explanation}
                                        </p>
                                    )}
                                </div>
